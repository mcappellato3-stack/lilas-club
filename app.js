...
function profileFromRaw(p,i){
  ...
  const gallery=(runtime.gallery||p.fotos||[]).filter(Boolean);
  const video=runtime.video||p.video||((p.videos||[])[0])||"";
  return {
    ...
    gallery,video,audio:runtime.audio||p.audioUrl||"",
    prices:{
...
async function getMediaBlob(key){if(!key)return null;try{const db=await openMediaDB();return await new Promise((resolve,reject)=>{const tx=db.transaction('assets','readonly');const q=tx.objectStore('assets').get(key);q.onsuccess=()=>resolve(q.result||null);q.onerror=()=>reject(q.error)})}catch{return null}}
